...
module.exports = class FritterApp {
  ...
  async loadFeedPosts (viewedProfile, opts = {}) {
    ...
    posts = await Promise.all(posts.map(async p => {
      p._threadParent = p.threadParent
      if (p.threadParent) {
  ...
  gotoParent (post, e) {
    ...
    if (post.threadParent && typeof post.threadParent === 'object') {
      window.history.pushState({}, null, this.threadUrl(post.threadParent))
    } else {
      const url = post.threadParent || post._threadParent;
      window.history.pushState({}, null, '/thread/' + url)
    }
  }
