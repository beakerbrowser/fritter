...
module.exports = class FritterApp {
  ...
  async setup () {
    ...
    this.router.on('/thread/:url', p => this.setView('thread', p.url))
    this.router.on('/user/:url', p => this.setView('user', p.url))
    this.router.on('/404', p => this.setView('404'))
    ...
    // TODO
    // replace this with a real local-user selection process
    // -prf
    this.currentUser = new DatArchive('dat://15ca2995666ea7fe9bbc30c4b5915c0feafc6d68ecedf6ee3b0574d57fc384d7/')
  ...
  async gotoProfile (profile) {
    window.history.pushState({}, null, '/user/' + profile.getRecordOrigin())
  }
}
