...
const themeColors = {
  ...
  boxShadow: Object.assign({}, themeColor, {l: 90, a: .75}),
  faded: Object.assign({}, themeColor, {l: 97})
}

const whoToFollowUrls = [
  'dat://232ac2ce8ad4ed80bd1b6de4cbea7d7b0cad1441fa62312c57a6088394717e41',
  'dat://4de73aa370f58291c7ad78d598ae31dcde3196a501d289f9ad0b8d76ed044855'
]
...
module.exports = class FritterApp {
  constructor () {
    ...
    this.whoToFollow = [] // hard-code a few fake profiles for sake of demonstration
  }
  ...
  async setup () {
    ...
    // set up who to follow profiles
    this.whoToFollow = await Promise.all(whoToFollowUrls.map(async (url) => {
      if (!this.isCurrentUserFollowing(url)) {
        await this.libfritter.db.indexArchive(url)
        return await this.libfritter.social.getProfile(url)
      }
    }))
    this.whoToFollow = this.whoToFollow.filter(Boolean)
    this.render()
